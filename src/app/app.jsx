      ...
      <div className="w-3/6 h-5/6 bg-gray-100 rounded-lg shadow-lg p-4 overflow-hidden relative">
          ...
          >
            {messages.map((currentMessage, currentMessageIndex) => (
              <div
                ...
                className={`flex flex-col mb-2 mt-6 ${
                  currentMessage.owner === socket.id ? 'items-end' : 'items-start'
                } `}
              >
                {currentMessage.owner === socket.id ? (
                  <div className="flex">
                    <div className="rounded-lg p-2 pl-4 bg-blue-500 relative">
                      ...
                      <div
                        style={{
                          background: currentMessage.reliability,
                          top: '0px',
                          height: '40px',
                          width: '10px',
                          borderRadius: '4px',
        ...
        {helps.length > 0 && (
          <div
            ...
            className="flex items-center ml-4 relative overflow-x-hidden w-full"
          >
            <div style={{ width: '5000px' }} className="flex items-center overflow-x-auto absolute">
              {helps.split('/').map((help, index) => (
                <div
                  ...
                  className="cursor-pointer mr-2 bg-blue-500 hover:bg-blue-600 text-white rounded-full px-4 py-2 focus:outline-none"
                  key={help + index}
                >
                  {help}
                </div>
        ...
        <div style={{ width: 'calc(100% - 64px)' }} className="flex items-center mt-4 ml-4 absolute bottom-[16px]">
          <div className="relative w-full">
            <input
              ...
              type="text"
              className="w-full flex-grow bg-gray-200 rounded-full px-4 py-2 text-gray-700 focus:outline-none"
              placeholder="Evoyer un message"
            />
            <button
              type="button"
              aria-label="Send"
              className="absolute top-1/2 -translate-y-1/2 right-[16px] rounded-full aspect-square p-2 text-white hover:bg-white hover:text-gray-60 dark:text-gray-600 dark:hover:bg-gray-600 dark:hover:text-white"
              title="Dictate your prompt"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 false">
          ...
          <button
            ...
            type="button"
            className="ml-2 bg-blue-500 hover:bg-blue-600 text-white rounded-full px-4 py-2 focus:outline-none"
          >
            Envoyer
          </button>
        </div>
